...
|-----------|----------|--------|
| `winscp` | `W` | WinSCP (SFTP / file transfer) |
| `terminal` | `T` | Windows Terminal + OpenSSH |
| `both` | `B` | WinSCP and Windows Terminal |
...
4. **Three actions only**: WinSCP, Windows Terminal, Open both. The combined action must show the **real WinSCP and Windows Terminal icons** together.
5. **CLI contract**: accept a 1Password SSH Bookmark URL as the first meaningful argument (`ssh://...`). 1Password is configured as: `"…\SSH-Launcher.exe" %s`.
6. **Never handle private keys**. Auth stays with the 1Password SSH Agent; this app only launches tools with the parsed host/user/port.
7. **i18n**: English (`en-US`) and Simplified Chinese (`zh-CN`). Default from system/WebView language; user can switch; preference in `localStorage` key `ssh-launcher-locale`.
8. **Theme**: light / dark. Default **follow system**. Override via CLI (`--theme=system|light|dark`, `--dark`, `--light`). Not a durable in-app toggle unless product decision changes.

## Architecture

```
┌──────────────────────────── Tauri 2 (Windows) ────────────────────────────┐
│  CLI args → parse ssh:// + theme → AppState(ConnectionInfo, theme)         │
│  Commands: get_connection_info | get_theme_preference | get_app_icons | launch_choice │
│  Process launch: WinSCP.exe / wt.exe + ssh.exe (CREATE_NO_WINDOW where needed) │
└───────────────────────────────────┬────────────────────────────────────────┘
                                    │ invoke()
┌───────────────────────────────────▼────────────────────────────────────────┐
...
| Layer | Path | Role |
|-------|------|------|
| Frontend UI | `src/App.jsx`, `src/styles.css`, `src/i18n.js`, `src/theme.js`, `src/main.jsx` | Chooser UI, shortcuts, locale, theme |
| Desktop backend | `src-tauri/src/main.rs` | URL/theme parse, icon extract, process launch |
...
- `get_app_icons` → `{ winscp?, terminal? }` as data-URL images from installed apps
- `launch_choice({ choice })` → launches then closes the window; `choice` is `winscp` \| `terminal` \| `both`
...
- **Icons**: prefer live icons from installed WinSCP / Windows Terminal via backend; Fluent UI icons are fallbacks only. Combined action uses stacked real icons.
- **Keyboard**: `W` / `T` / `B` / `Esc`. Cards show the shortcut as `<kbd>`.
...
- [ ] Combined action shows both real app icons when apps are installed
- [ ] zh-CN and en-US remain complete for new strings
- [ ] Browser preview still shows demo data without Tauri
...
- Extra launch targets beyond WinSCP and Windows Terminal
- Resizable or multi-window UI
- In-window theme picker (theme is system-default or CLI-only unless product decision changes)
